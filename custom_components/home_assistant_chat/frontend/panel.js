...
const largeB64 = (value) => { const bytes=new Uint8Array(value); let binary=""; for (let offset=0;offset<bytes.length;offset+=0x8000) binary+=String.fromCharCode(...bytes.subarray(offset,offset+0x8000)); return btoa(binary); };
...
const STRINGS = {
  en: {
    ...
    security:"Security code", deviceCount:"Devices", enabled:"Chat enabled", allowUsers:"Allow users", allowAttachments:"Allow photo and video attachments", encryption:"Experimental encryption",
    ...
    messageDeleted:"Message deleted", serverMessage:"Server", showDeletedMessages:"Show a marker for deleted messages", adminsDeleteMessages:"Allow administrators to delete other users’ messages", userSettingsHint:"Manage the people you have blocked.",
    ...
    resetKey:"Start with a new key", confirmResetKey:"Create a new encryption key for this chat? Older messages whose keys cannot be recovered will remain unreadable, but new messages will work.", resetError:"Could not create a new encryption key. Please try again.", attach:"Attach photo or video", photo:"Photo", video:"Video", loadMedia:"Load media", loadingMedia:"Loading photo…", downloadMedia:"Download", uploading:"Encrypting and uploading…", attachmentTooLarge:"Photos and videos may be up to 25 MB.", attachmentInvalid:"Choose a photo or video file.", attachmentFailed:"The attachment could not be sent. Please try again.",
    offlineSetupRequired:"This contact has not registered a Chat device yet. They need to open Chat once before encrypted offline delivery can work.", offlineDeliveryFailed:"The encrypted key could not be stored for every recipient device. The message was not sent; please try again."
  },
  de: {
    ...
    deviceCount:"Geräte", enabled:"Chat aktiviert", allowUsers:"Benutzer zulassen", allowAttachments:"Foto- und Videoanhänge erlauben", encryption:"Experimentelle Verschlüsselung",
    ...
    messageDeleted:"Nachricht gelöscht", serverMessage:"Server", showDeletedMessages:"Markierung für gelöschte Nachrichten anzeigen", adminsDeleteMessages:"Administratoren dürfen Nachrichten anderer Benutzer löschen", userSettingsHint:"Verwalte die von dir blockierten Benutzer.",
    ...
    resetKey:"Mit neuem Schlüssel fortfahren", confirmResetKey:"Einen neuen Verschlüsselungsschlüssel für diesen Chat erstellen? Ältere Nachrichten ohne wiederherstellbaren Schlüssel bleiben unlesbar, aber neue Nachrichten funktionieren wieder.", resetError:"Der neue Verschlüsselungsschlüssel konnte nicht erstellt werden. Bitte versuche es erneut.", attach:"Foto oder Video anhängen", photo:"Foto", video:"Video", loadMedia:"Medium laden", loadingMedia:"Foto wird geladen…", downloadMedia:"Herunterladen", uploading:"Wird verschlüsselt und hochgeladen…", attachmentTooLarge:"Fotos und Videos dürfen höchstens 25 MB groß sein.", attachmentInvalid:"Wähle eine Foto- oder Videodatei aus.", attachmentFailed:"Der Anhang konnte nicht gesendet werden. Bitte versuche es erneut.",
    offlineSetupRequired:"Dieser Kontakt hat noch kein Chat-Gerät registriert. Die Person muss Chat einmal öffnen, bevor verschlüsselte Offline-Zustellung funktioniert.", offlineDeliveryFailed:"Der verschlüsselte Schlüssel konnte nicht für alle Empfängergeräte gespeichert werden. Die Nachricht wurde nicht gesendet; bitte versuche es erneut."
  }
};
...
class HomeAssistantChatPanel extends HTMLElement {
  ...
  disconnectedCallback() { this._disconnected = true; this.stopQrScanner(); this.teardownViewport(); for (const item of this._attachmentUrls?.values() || []) URL.revokeObjectURL(item.url); this._attachmentUrls?.clear(); if (this._keySyncTimer) { clearInterval(this._keySyncTimer); this._keySyncTimer = null; } if (this._retryTimer) { clearTimeout(this._retryTimer); this._retryTimer = null; } if (this._encryptionRetryTimer) { clearTimeout(this._encryptionRetryTimer); this._encryptionRetryTimer = null; } if (typeof this._unsubscribe === "function") this._unsubscribe(); this._unsubscribe = null; this._ready = false; this._coreReady = false; }
  ...
  async refreshState() {
    ...
    if (previousScope && previousScope !== this._storageScope) {
      ...
      for (const item of this._attachmentUrls?.values() || []) URL.revokeObjectURL(item.url); this._attachmentUrls = new Map();
    ...
    const liveAttachments=new Set(this._messages.map((message) => message.attachment_id).filter(Boolean)); for (const [id,item] of this._attachmentUrls?.entries() || []) if (!liveAttachments.has(id)) { URL.revokeObjectURL(item.url); this._attachmentUrls.delete(id); }
  ...
  async shareKey(channelId, epoch) {
    const result={targets:0,peerTargets:0,delivered:0,peerDelivered:0,failed:0,peerFailed:0,lookupFailed:false};
    const scope=this._storageScope || activeStorageScope; const key = await channelKey(channelId, epoch, false, scope); if (!key) { result.lookupFailed=true; return result; }
    try {
      ...
      for (const target of await this.ws({type:"home_assistant_chat/key/devices", channel_id:channelId})) {
        ...
        const peer=target.user_id !== this._state?.user_id; result.targets += 1; if (peer) result.peerTargets += 1;
        try { await this.ws({type:"home_assistant_chat/key/offer", channel_id:channelId, device_id:target.id, key_id:`${channelId}:${epoch}`, key_commitment:commitment, wrapped_key:await wrapChannelKey(key, target, scope)}); result.delivered += 1; if (peer) result.peerDelivered += 1; }
        catch (error) {
          // A conflict means this sender already deposited an offer for that device.
          if (String(error?.code || error?.message || error).includes("key_offer_conflict")) { result.delivered += 1; if (peer) result.peerDelivered += 1; }
          else { result.failed += 1; if (peer) result.peerFailed += 1; }
        }
      }
    } catch { result.lookupFailed=true; }
    return result;
  }
  ...
  attachmentMetadata(value, message=null) {
    try {
      const item=JSON.parse(value); const validId=/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(item?.id || ""); const validMime=/^(?:image|video)\/[a-z0-9.+-]{1,100}$/i.test(item?.mime || ""); const validName=item?.name===undefined || (typeof item.name==="string" && item.name.length>0 && item.name.length<=255); const validSize=Number.isSafeInteger(item?.size) && item.size>0 && item.size<=25*1024*1024; const validNonce=typeof item?.nonce==="string" && raw(item.nonce).length===12;
      return item?.v===1 && item?.type==="attachment" && validId && validMime && validName && validSize && validNonce && message?.attachment_id===item.id ? item : null;
    }
    catch { return null; }
  }

  messageBodyMarkup(value, message) {
    const item=this.attachmentMetadata(value,message); if (!item) return esc(value);
    const cached=this._attachmentUrls?.get(item.id); const image=String(item.mime).startsWith("image/"); const label=image ? this.text.photo : this.text.video;
    if (cached) return `<div class="attachment"><div class="attachment-heading"><strong>${label}</strong><details class="attachment-menu"><summary aria-label="${esc(this.text.moreOptions)}" title="${esc(this.text.moreOptions)}"><ha-icon icon="mdi:dots-vertical"></ha-icon></summary><div class="attachment-menu-popover"><a class="button" href="${esc(cached.url)}" download="${esc(this.attachmentDownloadName(item))}">${this.text.downloadMedia}</a></div></details></div>${image ? `<img src="${esc(cached.url)}" alt="${label}">` : `<video src="${esc(cached.url)}" controls playsinline preload="metadata"></video>`}</div>`;
    if (image && !this._attachmentAutoFailed?.has(item.id)) return `<div class="attachment attachment-auto"><strong>${label}</strong><small>${this.text.loadingMedia}</small></div>`;
    return `<div class="attachment"><strong>${label}</strong><small>${Math.ceil(Number(item.size || 0)/1024)} KB</small><button class="button load-attachment" data-id="${esc(item.id)}">${this.text.loadMedia}</button></div>`;
  }

  attachmentDownloadName(item) { const subtype=String(item.mime || "").split("/")[1]?.split(/[;+]/)[0].replace(/[^a-z0-9]/gi,"").toLowerCase() || "bin"; const mapped={jpeg:"jpg",quicktime:"mov",xmatroska:"mkv"}[subtype] || subtype; return `${String(item.mime).startsWith("image/") ? "photo" : "video"}.${mapped}`; }

  bindAttachmentButton(container, message) {
    const item=this.attachmentMetadata(this._plaintext?.get(message.id),message); if (item && String(item.mime).startsWith("image/") && !this._attachmentUrls?.has(item.id) && !this._attachmentAutoFailed?.has(item.id)) { this.queueAutomaticAttachment(message,container); return; }
    container.querySelector(".load-attachment")?.addEventListener("click", async (event) => {
      const button=event.currentTarget; button.disabled=true;
      try { await this.loadAttachment(message); const value=this._plaintext?.get(message.id); container.innerHTML=this.messageBodyMarkup(value,message); }
      catch { button.disabled=false; this._error=this.text.attachmentFailed; this.render(); }
    });
  }

  queueAutomaticAttachment(message, container) {
    const item=this.attachmentMetadata(this._plaintext?.get(message.id),message); if (!item) return; this._attachmentAutoQueued ||= new Set(); if (this._attachmentAutoQueued.has(item.id)) return;
    this._attachmentAutoQueued.add(item.id); this._attachmentAutoQueue ||= []; this._attachmentAutoQueue.push({message,container,item}); this.processAutomaticAttachments();
  }

  processAutomaticAttachments() {
    this._attachmentAutoActive ||= 0;
    while (this._attachmentAutoActive<2 && this._attachmentAutoQueue?.length) {
      const job=this._attachmentAutoQueue.shift(); this._attachmentAutoActive += 1;
      this.loadAttachment(job.message).then(() => { if (job.container.isConnected) job.container.innerHTML=this.messageBodyMarkup(this._plaintext.get(job.message.id),job.message); }).catch(() => { this._attachmentAutoFailed ||= new Set(); this._attachmentAutoFailed.add(job.item.id); if (job.container.isConnected) { job.container.innerHTML=this.messageBodyMarkup(this._plaintext.get(job.message.id),job.message); this.bindAttachmentButton(job.container,job.message); } }).finally(() => { this._attachmentAutoQueued.delete(job.item.id); this._attachmentAutoActive -= 1; this.processAutomaticAttachments(); });
    }
  }

  async loadAttachment(message) {
    const item=this.attachmentMetadata(this._plaintext?.get(message.id),message); if (!item || this._attachmentUrls?.has(item.id)) return;
    const scope=this._storageScope;
    const chunks=[]; let offset=0,total=null;
    while (total===null || offset<total) { const response=await this.ws({type:"home_assistant_chat/attachment/get",attachment_id:item.id,offset}); const chunk=raw(response.data); chunks.push(chunk); offset=response.next_offset; total=response.size; if (!chunk.length && offset<total) throw new Error("attachment_incomplete"); }
    if (total!==item.size+16) throw new Error("attachment_size"); const encrypted=new Uint8Array(total); let position=0; for (const chunk of chunks) { encrypted.set(chunk,position); position+=chunk.length; }
    const epoch=message.envelope.key_id.split(":").pop(); const key=await channelKey(message.channel_id,epoch,false,scope || activeStorageScope); if (!key) throw new Error("missing_key");
    const bytes=await crypto.subtle.decrypt({name:"AES-GCM",iv:raw(item.nonce),additionalData:encoder.encode(`${message.channel_id}:${item.id}`)},key,encrypted);
    if (bytes.byteLength!==item.size || scope!==this._storageScope || this._disconnected) throw new Error("attachment_context"); const url=URL.createObjectURL(new Blob([bytes],{type:item.mime || "application/octet-stream"})); this._attachmentUrls ||= new Map(); this._attachmentUrls.set(item.id,{url});
  }

  async sendAttachment(file) {
    const channel=this._channels.find((item) => item.id===this._active); if (!channel || this.cannotPost(channel) || this._uploading) return;
    if (!file || (!String(file.type).startsWith("image/") && !String(file.type).startsWith("video/"))) { this._error=this.text.attachmentInvalid; this.render(); return; }
    if (!file.size || file.size>25*1024*1024) { this._error=this.text.attachmentTooLarge; this.render(); return; }
    this._uploading=true; this._error=this.text.uploading; this.render(); const epoch=channel.key_epoch || 1;
    try {
      const scope=this._storageScope || activeStorageScope; const state=await this.ws({type:"home_assistant_chat/key/state",channel_id:channel.id}); let key=await channelKey(channel.id,epoch,false,scope);
      if (key && state.commitment && await channelKeyCommitment(key)!==state.commitment) { await dbDeleteForScope(`key:${channel.id}:${epoch}`,scope); key=null; }
      if (!key && this._messages.some((message) => message.channel_id===channel.id)) { this._waiting=true; await this.updateKeyState(channel.id,true); throw new Error("missing_key"); }
      key ||= await channelKey(channel.id,epoch,true,scope); const attachmentId=crypto.randomUUID(); const nonce=crypto.getRandomValues(new Uint8Array(12));
      if (channel.kind==="private") { const delivery=await this.shareKey(channel.id,epoch); if (delivery.lookupFailed || delivery.peerFailed) throw new Error("offline_delivery"); if (!delivery.peerTargets) throw new Error("offline_setup"); }
      const encrypted=new Uint8Array(await crypto.subtle.encrypt({name:"AES-GCM",iv:nonce,additionalData:encoder.encode(`${channel.id}:${attachmentId}`)},key,await file.arrayBuffer()));
      const started=await this.ws({type:"home_assistant_chat/attachment/start",channel_id:channel.id,attachment_id:attachmentId,size:encrypted.length}); const chunkSize=Number(started.chunk_size)||196608;
      for (let offset=0;offset<encrypted.length;offset+=chunkSize) await this.ws({type:"home_assistant_chat/attachment/chunk",attachment_id:attachmentId,offset,data:largeB64(encrypted.subarray(offset,Math.min(offset+chunkSize,encrypted.length)))});
      await this.ws({type:"home_assistant_chat/attachment/finish",attachment_id:attachmentId});
      const metadata=JSON.stringify({v:1,type:"attachment",id:attachmentId,mime:String(file.type).slice(0,127),size:file.size,nonce:b64(nonce)}); const envelope=await encryptMessage(channel.id,epoch,metadata,scope);
      const sent=await this.ws({type:"home_assistant_chat/send",channel_id:channel.id,attachment_id:attachmentId,...envelope}); if (sent?.message?.id) { this._plaintext ||= new Map(); this._plaintext.set(sent.message.id,metadata); }
      if (channel.kind!=="private") await this.shareKey(channel.id,epoch); await this.updateRecoveryBundle(); this._error=""; await this.refreshState();
    } catch (error) { const code=String(error?.code || error?.message); this._error=code.includes("offline_setup") ? this.text.offlineSetupRequired : code.includes("offline_delivery") ? this.text.offlineDeliveryFailed : this.text.attachmentFailed; this.render(); }
    finally { this._uploading=false; this.render(); }
  }
  ...
  groupParticipantDialog(message) {
    if (!message?.sender_id || message.sender_id === this._state?.user_id) return;
    const identity=String(message.sender_identity || ""); const qrIdentity=String(message.sender_identity_address || identity);
    const existing=(this._channels || []).find((channel) => channel.kind === "private" && channel.peer?.id === message.sender_id);
    const dialog=this.dialog(this.text.contactInfo, `<div class="row"><span>${this.text.name}</span><strong>${esc(message.sender_name || "User")}</strong></div><div class="row"><span>${this.text.identityNumber}</span><code>${esc(identity || "—")}</code></div>`, `${identity ? `<button class="button group-peer-qr">${this.text.showQr}</button><button class="button group-peer-private">${this.text.private}</button>` : ""}${existing ? `<button class="button group-peer-silence">${existing.silenced ? this.text.unsilence : this.text.silence}</button><button class="button danger group-peer-delete">${this.text.deleteBoth}</button>` : ""}<button class="button danger group-peer-block">${this.text.block}</button><button class="button close">${this.text.close}</button>`);
    dialog.querySelector(".close").addEventListener("click", () => dialog.remove());
    dialog.querySelector(".group-peer-qr")?.addEventListener("click", () => this.showIdentityQr(qrIdentity));
    dialog.querySelector(".group-peer-private")?.addEventListener("click", async () => {
      try { const result=await this.ws({type:"home_assistant_chat/private",handle:identity,confirm_unblock:false}); dialog.remove(); await this.refreshState(); await this.selectChannel(result.channel.id); }
      catch (error) {
        dialog.remove();
        if (String(error?.code || error?.message).includes("confirm_unblock")) this.confirmDialog(this.text.confirmUnblock,this.text.continue,async () => { const result=await this.ws({type:"home_assistant_chat/private",handle:identity,confirm_unblock:true}); await this.refreshState(); await this.selectChannel(result.channel.id); });
        else { this._error=this.text.unavailable; this.render(); }
      }
    });
    dialog.querySelector(".group-peer-silence")?.addEventListener("click", async () => { dialog.remove(); await this.togglePrivateSilence(existing); });
    dialog.querySelector(".group-peer-delete")?.addEventListener("click", () => { dialog.remove(); this.deletePrivate(existing); });
    dialog.querySelector(".group-peer-block").addEventListener("click", () => { dialog.remove(); this.blockPrivate({peer:{id:message.sender_id}}); });
  }
  ...
  async adminDialog() {
    ...
    const show = async (tab) => {
      if (tab === "channels") {
        ...
        content.querySelector(".save-settings").addEventListener("click", async () => { await this.ws({type:"home_assistant_chat/settings", enabled:content.querySelector("#setting-enabled").checked, allow_users:content.querySelector("#setting-users").checked, attachments_enabled:content.querySelector("#setting-attachments").checked, admins_can_delete_messages:content.querySelector("#setting-admin-delete").checked, federation_qr_enabled:content.querySelector("#setting-federation-qr").checked, federation_address:content.querySelector("#setting-federation-address").value.trim(), federation_port:Number(content.querySelector("#setting-federation-port").value || 0), retention_days:Number(content.querySelector("#setting-retention").value), show_security_details:content.querySelector("#setting-security").checked, show_deleted_messages:content.querySelector("#setting-deleted-markers").checked}); await reload(); await show("settings"); });
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>:host{display:block;height:100dvh;min-height:0;overflow:hidden;background:var(--primary-background-color,#11151b);color:var(--primary-text-color,#e7e9ed);font:14px system-ui}*{box-sizing:border-box}main{--chat-header-height:104px;height:100%;min-height:0;display:grid;grid-template-columns:250px minmax(0,1fr);max-width:1200px;margin:auto}.side{min-height:0;border-right:1px solid var(--divider-color,#2d3540);overflow:auto}.side-top{height:var(--chat-header-height);padding:14px 12px;display:flex;align-items:center;justify-content:space-between;gap:8px;border-bottom:1px solid var(--divider-color,#2d3540)}.side-top h2{margin:0}.side-top .button{display:grid;place-items:center;width:34px;height:34px;padding:0;font-size:20px;line-height:1}.channel-list{padding:14px 12px}.channel-row{display:flex;align-items:center}.channel-row .channel{flex:1;min-width:0}.channel{display:block;width:100%;text-align:left;background:none;border:0;color:inherit;padding:10px;border-radius:8px;cursor:pointer}.active{background:var(--secondary-background-color,#2d3748)}.content{display:flex;flex-direction:column;min-width:0;min-height:0}.top{height:var(--chat-header-height);padding:14px 18px;border-bottom:1px solid var(--divider-color,#2d3540)}.topline,.header-actions,.chat-heading,.message-head,.row,.actions{display:flex;align-items:center;gap:8px}.topline,.message-head,.row{justify-content:space-between}.header-actions{justify-content:flex-end;flex-wrap:wrap}.top h3{margin:.25rem 0}.private-menu{position:relative}.private-menu summary{display:grid;place-items:center;width:34px;height:34px;border-radius:8px;cursor:pointer;list-style:none}.private-menu summary::-webkit-details-marker{display:none}.private-menu summary:hover{background:var(--secondary-background-color,#2d3748)}.private-menu-popover{position:absolute;top:38px;left:0;z-index:10;display:grid;min-width:190px;padding:6px;border:1px solid var(--divider-color,#465365);border-radius:10px;background:var(--card-background-color,#202a36);box-shadow:0 8px 24px #0007}.sidebar-private-menu .private-menu-popover{left:auto;right:0}.private-menu-popover .button{border:0;text-align:left;background:transparent}.peer-avatar{display:inline-grid;place-items:center;width:34px;height:34px;border-radius:50%;background:var(--accent-color,#03a9f4);color:#fff;font-weight:700}.security{margin:.35rem 0 0;color:var(--success-color,#58c28b);font-size:.85rem}.messages{flex:1;min-height:0;overflow:auto;padding:18px}.message{padding:10px;margin:7px 0;background:var(--card-background-color,#202a36);border-radius:10px;max-width:78%;overflow-wrap:anywhere}.message.deleted{opacity:.7;font-style:italic}.message-head{align-items:flex-start;margin-bottom:4px}.message-head small{min-width:0;padding-top:4px}.message-sender{border:0;padding:0;background:transparent;color:inherit;font:inherit;text-decoration:underline;text-underline-offset:2px;cursor:pointer}.mine{margin-left:auto;background:#194e5c}.announcement-message{margin-left:auto;margin-right:auto;background:var(--secondary-background-color,#2d3748)}.compose{display:flex;flex:0 0 auto;gap:8px;padding:12px;border-top:1px solid var(--divider-color,#2d3540)}.compose input{flex:1;min-width:0}.button,input,select{border:1px solid var(--divider-color,#465365);border-radius:8px;background:var(--card-background-color,#1b232d);color:inherit;padding:9px}.button{cursor:pointer}.button.delete-message{display:grid;place-items:center;flex:0 0 28px;width:28px;height:28px;padding:0;border:0;background:transparent}.button.delete-message:hover{background:color-mix(in srgb,var(--error-color,#ffb4ab) 12%,transparent)}.delete-message ha-icon{--mdc-icon-size:18px}.button:disabled,input:disabled{opacity:.55;cursor:not-allowed}.danger{color:var(--error-color,#ffb4ab)}.modal{position:fixed;inset:0;z-index:20;background:#0009;display:grid;place-items:center;padding:20px}.dialog{width:min(700px,100%);max-height:90vh;overflow:auto;background:var(--card-background-color,#202a36);border:1px solid var(--divider-color,#4a5868);border-radius:14px;padding:20px}.actions{justify-content:flex-end;margin-top:16px}.tabs{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px}.tabs button{background:var(--secondary-background-color,#2d3748);color:inherit;border:0;border-radius:7px;padding:8px;cursor:pointer}.row{padding:8px;border-bottom:1px solid var(--divider-color,#35404d)}label{display:grid;gap:5px;margin:.6rem 0}select[multiple]{min-height:9rem}@media(max-width:700px){main{--chat-header-height:auto;grid-template-columns:1fr;grid-template-rows:auto minmax(0,1fr)}.side{border-right:0}.side-top{height:auto}.channel-list{display:flex;gap:4px;overflow:auto;border-bottom:1px solid var(--divider-color,#2d3540)}.channel-row{flex:0 0 auto}.channel{white-space:nowrap;width:auto}.top{height:auto}.message{max-width:92%}.topline{align-items:flex-start;flex-direction:column}.header-actions{justify-content:flex-start}}</style><main>
      ...
      <div class="messages">${this._hasOlderMessages ? `<button class="button load-older" type="button">${text.loadOlder}</button>` : ""}${this._waiting ? `<p>${text.waiting} <button class="button" id="reset-key">${text.resetKey}</button></p>` : ""}${this._error ? `<p>${esc(this._error)}</p>` : ""}${this._encryptionError ? `<p>${esc(this._encryptionError)}</p>` : ""}${messages.length ? messages.map((message) => `<article class="message ${current?.kind !== "announcement" && message.sender_id === this._state?.user_id ? "mine" : ""} ${current?.kind === "announcement" ? "announcement-message" : ""} ${message.deleted ? "deleted" : ""}" data-message="${esc(message.id)}"><div class="message-head"><small>${current?.kind === "group" && message.sender_id !== this._state?.user_id ? `<button class="message-sender" data-sender-id="${esc(message.sender_id)}">${esc(message.sender_name || "User")}</button>` : esc(current?.kind === "announcement" ? text.serverMessage : message.sender_name || "User")} · ${new Date(message.created * 1000).toLocaleString(this.language === "de" ? "de-DE" : "en-US")}</small>${!message.deleted && message.sender_id === this._state?.user_id ? `<button class="button danger delete-message" data-id="${esc(message.id)}" aria-label="${esc(text.delete)}" title="${esc(text.delete)}"><ha-icon icon="mdi:delete-outline" aria-hidden="true"></ha-icon></button>` : ""}</div><span class="body">${message.deleted ? text.messageDeleted : this._plaintext?.has(message.id) ? this.messageBodyMarkup(this._plaintext.get(message.id),message) : text.encrypted}</span></article>`).join("") : `<p>${text.noMessages}</p>`}</div>
      <form class="compose">${this._state?.settings?.attachments_enabled ? `<input class="attachment-input" type="file" accept="image/*,video/*" hidden><button class="button attach-button" type="button" aria-label="${esc(text.attach)}" title="${esc(text.attach)}" ${readOnly || this._uploading ? "disabled" : ""}><ha-icon icon="mdi:paperclip"></ha-icon></button>` : ""}<input id="message-input" maxlength="4000" autocomplete="off" placeholder="${esc(readOnly ? text.cannotPost : text.write)}" ${readOnly || this._uploading ? "disabled" : ""}><button class="button" ${readOnly || this._uploading ? "disabled" : ""}>${text.send}</button></form></section></main>`;
    if (!this.shadowRoot.querySelector("#responsive-chat-style")) { const style=document.createElement("style"); style.id="responsive-chat-style"; style.textContent=".ha-sidebar-toggle{display:none}@media (max-width:700px){:host{height:100dvh;overflow:hidden}main{width:100%;height:100dvh;grid-template-columns:1fr;grid-template-rows:auto minmax(0,1fr);overflow:hidden}.side{display:grid;grid-template-columns:104px minmax(0,1fr);align-items:center;min-width:0;overflow:hidden;border-right:0;border-bottom:1px solid var(--divider-color,#2d3540);background:var(--card-background-color,#151b22)}.side-top{height:auto;padding:6px;border:0}.side-top h2{display:none}.side-top .button{width:44px;height:44px}.ha-sidebar-toggle{display:block;flex:0 0 44px;width:44px;height:44px}.channel-list{display:flex;gap:4px;padding:6px 8px;overflow-x:auto;overscroll-behavior-x:contain;border:0;scrollbar-width:none}.channel-list::-webkit-scrollbar{display:none}.channel-row{flex:0 0 auto}.channel{width:auto;white-space:nowrap;padding:10px 12px}.content{min-height:0;overflow:hidden}.top{height:auto;min-height:56px;padding:8px 10px}.topline{flex-direction:row;align-items:center;gap:6px}.chat-heading{min-width:0}.chat-heading h3{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.header-actions{flex-wrap:nowrap;margin-left:auto}.header-actions .button{padding:8px}.security{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.messages{padding:10px;overscroll-behavior:contain}.message{max-width:94%;padding:8px 10px;margin:5px 0}.compose{position:relative;z-index:2;padding:8px 8px calc(8px + env(safe-area-inset-bottom));min-height:60px;background:var(--primary-background-color,#11151b)}.compose input,.compose .button{min-height:44px;font-size:16px}.modal{padding:0;overflow:hidden;place-items:end stretch}.dialog{width:100%;max-height:calc(100dvh - 16px);margin:0;padding:16px;border-radius:16px 16px 0 0;overflow:auto}.private-menu-popover{position:fixed;top:auto;right:8px;bottom:calc(68px + env(safe-area-inset-bottom));left:8px;max-width:none}}"; this.shadowRoot.append(style); }
    ...
    if (!this.shadowRoot.querySelector("#attachment-chat-style")) { const style=document.createElement("style"); style.id="attachment-chat-style"; style.textContent=".attach-button{display:grid;place-items:center;flex:0 0 42px;padding:0}.attachment{display:grid;gap:8px}.attachment-heading{display:flex;align-items:center;justify-content:space-between;gap:8px}.attachment img,.attachment video{display:block;width:100%;max-width:520px;max-height:60vh;object-fit:contain;border-radius:8px;background:#000}.attachment .button,.attachment a{justify-self:start}.attachment small{opacity:.75}.attachment-menu{position:relative}.attachment-menu summary{display:grid;place-items:center;width:30px;height:30px;border-radius:7px;cursor:pointer;list-style:none}.attachment-menu summary::-webkit-details-marker{display:none}.attachment-menu-popover{position:absolute;top:34px;right:0;z-index:12;display:grid;min-width:130px;padding:6px;border:1px solid var(--divider-color,#465365);border-radius:9px;background:var(--card-background-color,#202a36);box-shadow:0 8px 24px #0007}.attachment-menu-popover .button{border:0;background:transparent;text-decoration:none}"; this.shadowRoot.append(style); }
    ...
    const attachmentInput=this.shadowRoot.querySelector(".attachment-input"); this.shadowRoot.querySelector(".attach-button")?.addEventListener("click", () => attachmentInput?.click()); attachmentInput?.addEventListener("change", () => { const file=attachmentInput.files?.[0]; attachmentInput.value=""; if (file) this.sendAttachment(file); });
    ...
    const menuButton=this.shadowRoot.querySelector("#ha-sidebar-toggle"); if (menuButton) { menuButton.hass=this._hass; menuButton.narrow=true; }
    ...
    this.shadowRoot.querySelectorAll(".message-sender").forEach((button) => button.addEventListener("click", () => this.groupParticipantDialog(messages.find((message) => message.sender_id === button.dataset.senderId))));
    const renderScope=this._storageScope;
    this.shadowRoot.querySelectorAll("[data-message]:not(.deleted)").forEach(async (element) => { const message = messages.find((item) => item.id === element.dataset.message); if (!message) return; const body=element.querySelector(".body"); if (this._plaintext?.has(message.id)) { this.bindAttachmentButton(body,message); return; } const value=await decryptMessage(message,renderScope); if (value !== null && this._storageScope === renderScope) { this._plaintext ||= new Map(); this._plaintext.set(message.id,value); body.innerHTML=this.messageBodyMarkup(value,message); this.bindAttachmentButton(body,message); } });
    if (this._state?.is_admin && this._state?.settings?.admins_can_delete_messages) this.shadowRoot.querySelectorAll("[data-message]:not(.deleted)").forEach((element) => { if (element.querySelector(".delete-message")) return; const message=messages.find((item) => item.id===element.dataset.message); if (message) element.querySelector(".message-head")?.insertAdjacentHTML("beforeend",`<button class="button danger delete-message" data-id="${esc(message.id)}" aria-label="${esc(text.delete)}" title="${esc(text.delete)}"><ha-icon icon="mdi:delete-outline" aria-hidden="true"></ha-icon></button>`); });
