...
const STRINGS = {
  ...
  de: {
    ...
    resetKey:"Mit neuem Schlüssel fortfahren", confirmResetKey:"Einen neuen Verschlüsselungsschlüssel für diesen Chat erstellen? Ältere Nachrichten ohne wiederherstellbaren Schlüssel bleiben unlesbar, aber neue Nachrichten funktionieren wieder.", resetError:"Der neue Verschlüsselungsschlüssel konnte nicht erstellt werden. Bitte versuche es erneut.", attach:"Foto oder Video anhängen", loadMedia:"Medium laden", downloadMedia:"Herunterladen", uploading:"Wird verschlüsselt und hochgeladen…", attachmentTooLarge:"Fotos und Videos dürfen höchstens 25 MB groß sein.", attachmentInvalid:"Wähle eine Foto- oder Videodatei aus.", attachmentFailed:"Der Anhang konnte nicht gesendet werden. Bitte versuche es erneut.",
...
class HomeAssistantChatPanel extends HTMLElement {
  ...
  attachmentMetadata(value, message=null) {
    try {
      const item=JSON.parse(value); const validId=/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(item?.id || ""); const validMime=/^(?:image|video)\/[a-z0-9.+-]{1,100}$/i.test(item?.mime || ""); const validName=typeof item?.name==="string" && item.name.length>0 && item.name.length<=255; const validSize=Number.isSafeInteger(item?.size) && item.size>0 && item.size<=25*1024*1024; const validNonce=typeof item?.nonce==="string" && raw(item.nonce).length===12;
  ...
  messageBodyMarkup(value, message) {
    ...
    const cached=this._attachmentUrls?.get(item.id); const name=esc(item.name || this.text.attach); const mime=String(item.mime || "application/octet-stream");
    if (cached) return `<div class="attachment"><strong>${name}</strong>${mime.startsWith("image/") ? `<img src="${esc(cached.url)}" alt="${name}">` : mime.startsWith("video/") ? `<video src="${esc(cached.url)}" controls playsinline preload="metadata"></video>` : ""}<a class="button" href="${esc(cached.url)}" download="${name}">${this.text.downloadMedia}</a></div>`;
    return `<div class="attachment"><strong>${name}</strong><small>${Math.ceil(Number(item.size || 0)/1024)} KB</small><button class="button load-attachment" data-id="${esc(item.id)}">${this.text.loadMedia}</button></div>`;
  }
  ...
  async loadAttachment(message) {
    ...
    const epoch=message.envelope.key_id.split(":").pop(); const key=await channelKey(message.channel_id,epoch,false,this._storageScope || activeStorageScope); if (!key) throw new Error("missing_key");
    ...
    if (bytes.byteLength!==item.size) throw new Error("attachment_size"); const url=URL.createObjectURL(new Blob([bytes],{type:item.mime || "application/octet-stream"})); this._attachmentUrls ||= new Map(); this._attachmentUrls.set(item.id,{url});
  }
  ...
  async sendAttachment(file) {
    ...
    try {
      ...
      const metadata=JSON.stringify({v:1,type:"attachment",id:attachmentId,name:String(file.name || this.text.attach).slice(0,255),mime:String(file.type).slice(0,127),size:file.size,nonce:b64(nonce)}); const envelope=await encryptMessage(channel.id,epoch,metadata,scope);
  ...
  render() {
    ...
    if (!this.shadowRoot.querySelector("#attachment-chat-style")) { const style=document.createElement("style"); style.id="attachment-chat-style"; style.textContent=".attach-button{display:grid;place-items:center;flex:0 0 42px;padding:0}.attachment{display:grid;gap:8px}.attachment img,.attachment video{display:block;width:100%;max-width:520px;max-height:60vh;object-fit:contain;border-radius:8px;background:#000}.attachment .button,.attachment a{justify-self:start}.attachment small{opacity:.75}"; this.shadowRoot.append(style); }
