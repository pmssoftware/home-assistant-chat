...
const STRINGS = {
  en: {
    ...
    contactInfo:"Contact info", silence:"Silence", unsilence:"Unmute", userIdentifier:"Home Assistant user ID", moreOptions:"More options"
  },
  de: {
    ...
    contactInfo:"Kontaktinformationen", silence:"Stummschalten", unsilence:"Stummschaltung aufheben", userIdentifier:"Home-Assistant-Benutzer-ID", moreOptions:"Weitere Optionen"
  }
};
...
class HomeAssistantChatPanel extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>:host{display:block;height:100dvh;min-height:0;overflow:hidden;background:var(--primary-background-color,#11151b);color:var(--primary-text-color,#e7e9ed);font:14px system-ui}*{box-sizing:border-box}main{--chat-header-height:104px;height:100%;min-height:0;display:grid;grid-template-columns:250px minmax(0,1fr);max-width:1200px;margin:auto}.side{min-height:0;border-right:1px solid var(--divider-color,#2d3540);overflow:auto}.side-top{height:var(--chat-header-height);padding:14px 12px;display:flex;align-items:center;justify-content:space-between;gap:8px;border-bottom:1px solid var(--divider-color,#2d3540)}.side-top h2{margin:0}.side-top .button{padding:8px;white-space:nowrap}.channel-list{padding:14px 12px}.channel{display:block;width:100%;text-align:left;background:none;border:0;color:inherit;padding:10px;border-radius:8px;cursor:pointer}.active{background:var(--secondary-background-color,#2d3748)}.content{display:flex;flex-direction:column;min-width:0;min-height:0}.top{height:var(--chat-header-height);padding:14px 18px;border-bottom:1px solid var(--divider-color,#2d3540)}.topline,.header-actions,.chat-heading,.message-head,.row,.actions{display:flex;align-items:center;gap:8px}.topline,.message-head,.row{justify-content:space-between}.header-actions{justify-content:flex-end;flex-wrap:wrap}.top h3{margin:.25rem 0}.private-menu{position:relative}.private-menu summary{display:grid;place-items:center;width:34px;height:34px;border-radius:8px;cursor:pointer;list-style:none}.private-menu summary::-webkit-details-marker{display:none}.private-menu summary:hover{background:var(--secondary-background-color,#2d3748)}.private-menu-popover{position:absolute;top:38px;left:0;z-index:10;display:grid;min-width:190px;padding:6px;border:1px solid var(--divider-color,#465365);border-radius:10px;background:var(--card-background-color,#202a36);box-shadow:0 8px 24px #0007}.private-menu-popover .button{border:0;text-align:left;background:transparent}.peer-avatar{display:inline-grid;place-items:center;width:34px;height:34px;border-radius:50%;background:var(--accent-color,#03a9f4);color:#fff;font-weight:700}.security{margin:.35rem 0 0;color:var(--success-color,#58c28b);font-size:.85rem}.messages{flex:1;min-height:0;overflow:auto;padding:18px}.message{padding:10px;margin:7px 0;background:var(--card-background-color,#202a36);border-radius:10px;max-width:78%;overflow-wrap:anywhere}.message.deleted{opacity:.7;font-style:italic}.message-head{margin-bottom:4px}.message-head small{min-width:0}.delete-message{flex:0 0 auto;padding:4px 8px}.mine{margin-left:auto;background:#194e5c}.compose{display:flex;flex:0 0 auto;gap:8px;padding:12px;border-top:1px solid var(--divider-color,#2d3540)}.compose input{flex:1;min-width:0}.button,input,select{border:1px solid var(--divider-color,#465365);border-radius:8px;background:var(--card-background-color,#1b232d);color:inherit;padding:9px}.button{cursor:pointer}.button:disabled,input:disabled{opacity:.55;cursor:not-allowed}.danger{color:var(--error-color,#ffb4ab)}.modal{position:fixed;inset:0;z-index:20;background:#0009;display:grid;place-items:center;padding:20px}.dialog{width:min(700px,100%);max-height:90vh;overflow:auto;background:var(--card-background-color,#202a36);border:1px solid var(--divider-color,#4a5868);border-radius:14px;padding:20px}.actions{justify-content:flex-end;margin-top:16px}.tabs{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px}.tabs button{background:var(--secondary-background-color,#2d3748);color:inherit;border:0;border-radius:7px;padding:8px;cursor:pointer}.row{padding:8px;border-bottom:1px solid var(--divider-color,#35404d)}label{display:grid;gap:5px;margin:.6rem 0}select[multiple]{min-height:9rem}@media(max-width:700px){main{--chat-header-height:auto;grid-template-columns:1fr;grid-template-rows:auto minmax(0,1fr)}.side{border-right:0}.side-top{height:auto}.channel-list{display:flex;gap:4px;overflow:auto;border-bottom:1px solid var(--divider-color,#2d3540)}.channel{white-space:nowrap;width:auto}.top{height:auto}.message{max-width:92%}.topline{align-items:flex-start;flex-direction:column}.header-actions{justify-content:flex-start}}</style><main>
      <aside class="side"><div class="side-top"><h2>${text.chat}</h2><button class="button" id="new-private">＋ ${text.private}</button></div><div class="channel-list">${channels.map((channel) => `<button class="channel ${channel.id === this._active ? "active" : ""}" data-channel="${esc(channel.id)}">${esc(channel.name)}</button>`).join("")}</div></aside>
      <section class="content"><header class="top"><div class="topline"><div><div class="chat-heading"><h3>${esc(current?.name || text.chat)}</h3>${current?.kind === "private" ? `<details class="private-menu"><summary aria-label="${esc(text.moreOptions)}" title="${esc(text.moreOptions)}"><ha-icon icon="mdi:dots-vertical" aria-hidden="true"></ha-icon></summary><div class="private-menu-popover"><button class="button" id="private-info">${text.contactInfo}</button><button class="button" id="silence-private">${current.silenced ? text.unsilence : text.silence}</button><button class="button danger" id="block-private">${text.block}</button><button class="button danger" id="delete-private">${text.deleteBoth}</button></div></details>` : ""}</div>${peer ? `<span class="peer-avatar">${esc(initials(peer.name))}</span> ${esc(peer.name)}` : ""}</div><div class="header-actions">${this._state?.is_admin ? `<button class="button" id="admin">${text.admin}</button>` : `<button class="button" id="user-settings">${text.settings}</button>`}</div></div>${security}</header>
    ...
    this.shadowRoot.querySelector("#private-info")?.addEventListener("click", () => this.privateInfoDialog(current)); this.shadowRoot.querySelector("#silence-private")?.addEventListener("click", () => this.togglePrivateSilence(current)); this.shadowRoot.querySelector("#block-private")?.addEventListener("click", () => this.blockPrivate(current)); this.shadowRoot.querySelector("#delete-private")?.addEventListener("click", () => this.deletePrivate(current));
