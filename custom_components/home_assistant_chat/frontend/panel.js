...
class HomeAssistantChatPanel extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>:host{display:block;height:100%;background:var(--primary-background-color,#11151b);color:var(--primary-text-color,#e7e9ed);font:14px system-ui}*{box-sizing:border-box}main{height:100%;display:grid;grid-template-columns:250px 1fr;max-width:1200px;margin:auto}.side{border-right:1px solid var(--divider-color,#2d3540);padding:14px;overflow:auto}.channel{display:block;width:100%;text-align:left;background:none;border:0;color:inherit;padding:10px;border-radius:8px;cursor:pointer}.active{background:var(--secondary-background-color,#2d3748)}.content{display:flex;flex-direction:column;min-width:0}.top{padding:14px 18px;border-bottom:1px solid var(--divider-color,#2d3540)}.topline,.private-actions,.row,.actions{display:flex;align-items:center;gap:8px}.topline,.row{justify-content:space-between}.peer-avatar{display:inline-grid;place-items:center;width:34px;height:34px;border-radius:50%;background:var(--accent-color,#03a9f4);color:#fff;font-weight:700}.security{margin:.35rem 0 0;color:var(--success-color,#58c28b);font-size:.85rem}.messages{flex:1;overflow:auto;padding:18px}.message{padding:10px;margin:7px 0;background:var(--card-background-color,#202a36);border-radius:10px;max-width:78%;overflow-wrap:anywhere}.mine{margin-left:auto;background:#194e5c}.compose{display:flex;gap:8px;padding:12px;border-top:1px solid var(--divider-color,#2d3540)}.compose input{flex:1}.button,input,select{border:1px solid var(--divider-color,#465365);border-radius:8px;background:var(--card-background-color,#1b232d);color:inherit;padding:9px}.button{cursor:pointer}.button:disabled,input:disabled{opacity:.55;cursor:not-allowed}.danger{color:var(--error-color,#ffb4ab)}.modal{position:fixed;inset:0;z-index:20;background:#0009;display:grid;place-items:center;padding:20px}.dialog{width:min(700px,100%);max-height:90vh;overflow:auto;background:var(--card-background-color,#202a36);border:1px solid var(--divider-color,#4a5868);border-radius:14px;padding:20px}.actions{justify-content:flex-end;margin-top:16px}.tabs{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px}.tabs button{background:var(--secondary-background-color,#2d3748);color:inherit;border:0;border-radius:7px;padding:8px;cursor:pointer}.row{padding:8px;border-bottom:1px solid var(--divider-color,#35404d)}label{display:grid;gap:5px;margin:.6rem 0}select[multiple]{min-height:9rem}@media(max-width:700px){main{grid-template-columns:1fr}.side{border:0;border-bottom:1px solid var(--divider-color,#2d3540);display:flex;gap:4px;overflow:auto}.side h2{display:none}.channel{white-space:nowrap;width:auto}.message{max-width:92%}.topline{align-items:flex-start;flex-direction:column}}</style><main>
      <aside class="side"><h2>${text.chat}</h2>${channels.map((channel) => `<button class="channel ${channel.id === this._active ? "active" : ""}" data-channel="${esc(channel.id)}">${esc(channel.name)}</button>`).join("")}<button class="button" id="new-private">＋ ${text.private}</button>${this._state?.is_admin ? `<button class="button" id="admin">${text.admin}</button>` : ""}</aside>
      <section class="content"><header class="top"><div class="topline"><div><h3>${esc(current?.name || text.chat)}</h3>${peer ? `<span class="peer-avatar">${esc(initials(peer.name))}</span> ${esc(peer.name)}` : ""}</div>${current?.kind === "private" ? `<div class="private-actions"><button class="button danger" id="block-private">${text.block}</button><button class="button danger" id="delete-private">${text.deleteBoth}</button></div>` : ""}</div>${security}</header>
      <div class="messages">${this._waiting ? `<p>${text.waiting}</p>` : ""}${this._error ? `<p>${esc(this._error)}</p>` : ""}${messages.length ? messages.map((message) => `<article class="message ${message.sender_id === this._state?.user_id ? "mine" : ""}" data-message="${esc(message.id)}"><small>${esc(message.sender_name || "User")} · ${new Date(message.created * 1000).toLocaleString(this.language === "de" ? "de-DE" : "en-US")}</small><br><span class="body">${text.encrypted}</span>${message.sender_id === this._state?.user_id || this._state?.is_admin ? `<button class="button danger delete-message" data-id="${esc(message.id)}">${text.delete}</button>` : ""}</article>`).join("") : `<p>${text.noMessages}</p>`}</div>
